...
public class ArticleRealmController {

    public ArticleRealmController() {
    }
    ...
    public RealmResults<Article> getArticles(Realm realm) {
    ...
    public Article getArticle(Realm realm,String id) {

        return realm.where(Article.class).equalTo("id", id).findFirst();
    }

    //check if Recipe.class is empty
    public boolean hasArticles(Realm realm) {

        return !realm.allObjects(Article.class).isEmpty();
    }

    public boolean insertArticle(Realm realm,final Article article){
        try {
    ...
    public boolean insertArticles(Realm realm,final List<Article> articles){
        try {
            realm.executeTransaction(new Realm.Transaction() {
                @Override
                public void execute(Realm realm) {
